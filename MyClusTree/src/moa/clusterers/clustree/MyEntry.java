...
public class MyEntry {
     ...
     * @see #data
     */
    protected MyEntry(int numberDimensions,
                      MyNode node, long currentTime, MyEntry parentEntry, MyNode containerNode) {
     ...
     * @see #data
     */
    public MyEntry(int numberDimensions, ClusKernel cluster, long currentTime) {
        this(numberDimensions);
     ...
     * @param containerNode
     */
    protected MyEntry(int numberDimensions, ClusKernel cluster, long currentTime, MyEntry parentEntry, MyNode containerNode) {
        this(numberDimensions);
        this.parentEntry = parentEntry;
        this.data.add(cluster);
        this.node = containerNode;
     ...
     * @param other
     */
    protected MyEntry(MyEntry other) {
        ...
        if (other.getChild() != null)
            for (MyEntry e : other.getChild().getEntries()) {
                e.setParentEntry(this);
            }
        //usage = other.usage;
        usage = 0;

        /*MyEntry parent = parentEntry;
        while(parent != null) {
            parent.incrementUsage();
    ...
    public void updateUsage(int bias) {
        usage += bias;
    }
     ...
     * @see #data
     */
    public double calcDistance(MyEntry other) {
     ...
     */
    protected void initializeEntry(MyEntry other, long currentTime) {
        assert (this.isEmpty());
        ...
        //usage = other.usage;
        if (child != null) {
            for (MyEntry e : child.getEntries()) {
                e.setParentEntry(this);
            }
        }
    }
     ...
     * @see Kernel
     */
    protected void aggregateEntry(MyEntry other, long currentTime,
                                  double negLambda) {
     ...
     * @see Kernel#add(tree.Kernel)
     */
    protected void mergeWith(MyEntry other) {
    ...
    public MyEntry getParentEntry() {
        return parentEntry;
    }

    public void setParentEntry(MyEntry parent) {
     ...
     */
    protected void overwriteOldEntry(MyEntry newEntry) {
        ...
        if (newEntry.getChild() != null)
            for (MyEntry e : newEntry.getChild().getEntries())
                e.setParentEntry(this);
    ...
    protected void recalculateData() {
        MyNode currentChild = this.getChild();
        if (currentChild != null) {
            ClusKernel currentData = this.getData();
            currentData.clear();
            MyEntry[] entries = currentChild.getEntries();
            for (int i = 0; i < entries.length; i++) {
