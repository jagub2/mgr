...
public class MyNode {
     ...
     */
    public MyNode(int numberDimensions, int level) {
        this.level = level;

        this.entries = new MyEntry[NUMBER_ENTRIES];
        // Generate all entries when we generate a Node.
        // That no entry can be null makes it much easier to handle.
        for (int i = 0; i < NUMBER_ENTRIES; i++) {
            entries[i] = new MyEntry(numberDimensions);
            entries[i].setNode(this);
        }
    }

    public void setEntries(MyEntry[] newEntries) {
        entries = newEntries;
    }
     ...
     */
    protected MyNode(int numberDimensions, int numberClasses, int level,
                     boolean fakeRoot) {
     ...
     * @param argEntries
     */
    public MyNode(int numberDimensions, int level, MyEntry[] argEntries) {
        this.level = level;
     ...
     */
    protected MyEntry nearestEntry(MyEntry newEntry) {
     ...
     */
    public void addEntry(MyEntry newEntry, long currentTime) {
     ...
     */
    protected MyEntry getIrrelevantEntry(double threshold) {
        for (int i = 0; i < this.entries.length; i++) {
            MyEntry entry = this.entries[i];
     ...
     */
    protected int getLevel(MyClusTree tree) {
